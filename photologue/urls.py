...
# galleries
gallery_args = {'date_field': 'date_added', 'allow_empty': True, 'num_latest':45, 'queryset': Gallery.objects.filter(is_public=True), 'extra_context':{'sample_size':SAMPLE_SIZE}}
...
# photographs
photo_args = {'date_field': 'date_added', 'allow_empty': True, 'queryset': Photo.objects.filter(is_public=True).order_by('-date_added')}
urlpatterns += patterns('django.views.generic.date_based',
...
urlpatterns += patterns('django.views.generic.list_detail',
    url(r'^photo/(?P<object_id>\d+)/$', 'object_detail', {'queryset': Photo.objects.filter(is_public=True)}, name='pl-photo'),
