...
class Gallery(MetaModel):
    ...
    def public(self):
        return self.photos.filter(is_public=True).order_by('image')
...
class Photo(ImageModel):
    title = models.CharField(_('title'), max_length=100, blank=True, default='', unique=False)
    title_slug = models.SlugField(_('slug'), unique=False, blank=True, default='',
    ...
    def __unicode__(self):
        ...
        return self.date_added.strftime('%d.%m.%Y %H:%S')

    def get_title(self):
        return re.sub('^\d+\s*-\s*', '', self.title, re.I)
    ...
    def get_absolute_url(self):
        return reverse('pl-photo', args=[self.title_slug or self.pk])
