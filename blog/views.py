...
def post_list(request, page=0):
    ...
    return list_detail.object_list(
        ...
        paginate_by = 10,
        page = page,
        extra_context = { 'category_all': category_all,
...
def post_detail_short(request, id):
    """
    Redirect for sharing
    """
...
def with_tag(request, tag, object_id=None, page=1):
    ...
    entries = TaggedItem.objects.get_by_model(models.Post, query_tag)
    entries = entries.order_by('-created')

    return render_to_response("blog/with_tag.html", dict(tag=tag, entries=entries), context_instance=RequestContext(request))
